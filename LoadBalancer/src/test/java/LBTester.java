...
public class LBTester {
    ...
    @Test
    public void LBDefaultBehaviour() throws IllegalAccessException {
        LoadBalancer loadBalancer = new LoadBalancer();
        final ConsistentHashing consistentHashing = new ConsistentHashing(point -> (long) Math.abs(point.hashCode()) % 100, 1);
        ...
        loadBalancer.register(new Service(emailServiceId, new WeightedRoundRobin(), new String[]{"sendEmil", "addTemplate", "getSmsForUser"}));
        ...
        Node profileNode1 = loadBalancer.getHandler(new Request("r-123", profileServiceId, "addProfile"));
        Node profileNode2 = loadBalancer.getHandler(new Request("r-244", profileServiceId, "addProfile"));
        Node profileNode3 = loadBalancer.getHandler(new Request("r-659", profileServiceId, "addProfile"));
        Node profileNode4 = loadBalancer.getHandler(new Request("r-73", profileServiceId, "addProfile"));
        ...
        Node smsNode1 = loadBalancer.getHandler(new Request("r-124", smsServiceId, "addTemplate"));
        Node smsNode2 = loadBalancer.getHandler(new Request("r-1214", smsServiceId, "addTemplate"));
        Node smsNode3 = loadBalancer.getHandler(new Request("r-4", smsServiceId, "addTemplate"));
        Assert.assertEquals(sNode1, smsNode1);
        Assert.assertEquals(sNode2, smsNode2);
        Assert.assertEquals(sNode1, smsNode3);

        Node emailNode1 = loadBalancer.getHandler(new Request("r-1232", emailServiceId, "addTemplate"));
        Node emailNode2 = loadBalancer.getHandler(new Request("r-4134", emailServiceId, "addTemplate"));
        Node emailNode3 = loadBalancer.getHandler(new Request("r-23432", emailServiceId, "addTemplate"));
        Node emailNode4 = loadBalancer.getHandler(new Request("r-5345", emailServiceId, "addTemplate"));
